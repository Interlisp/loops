...
(defun emit-fetch-wrapper (metatype argument dest miss-label &optional slot)
  (let ((exit-emit-fetch-wrapper (gensym "exit-emit-fetch-wrapper")))
    (with-lap-registers ((arg t))
      (ecase metatype
	(standard-instance
	  (let ((get-std-inst-wrapper (gensym "get-std-inst-wrapper"))
		(get-fsc-inst-wrapper (gensym "get-fsc-inst-wrapper")))
	    (flatten-lap
	...
	(class
	  ...
	  (let ((get-std-inst-wrapper (gensym "get-std-inst-wrapper"))
		(get-fsc-inst-wrapper (gensym "get-fsc-inst-wrapper"))
		(get-built-in-wrapper (gensym "get-built-in-wrapper")))
	    (flatten-lap
	      (opcode :move (operand :arg argument) arg)
	...
	(built-in-instance
	  (when slot (error "Can't do a slot reg for this metatype."))
	  (let ((get-built-in-wrapper (gensym "get-built-in-wrapper")))
	    (flatten-lap
	      (opcode :move (operand :arg argument) arg)
	      (opcode :built-in-instance-p arg get-built-in-wrapper)
	      (opcode :go miss-label)
