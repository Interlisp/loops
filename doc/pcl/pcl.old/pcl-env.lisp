...
(defun prompt-for-full-method-name (gf-name &optional has-def-p)
  ...
  (let ((method-names (generic-function-method-names gf-name has-def-p)))
    (cond ((null method-names)
	  ...
	  (t (il:menu
	      (il:create
	       il:menu il:items il:¬	;If HAS-DEF-P, include only
...
(defun il:attach-backtrace-menu (&optional (il:ttywindow
  ...
  (let ((il:bkmenu (il:|create| il:menu
    ...
    (il:addmenu il:bkmenu il:btw (il:|create| il:¬ il:position
				     il:xcoord il:¬ 0
                            ...
                            (when xcl::interesting-p        
                                ...
                                (xcl::collect-item (il:|create| il:bkmenuitem
                                                          il:label il:¬ (let ((*print-level* 2)
                                                                             ...
                                                                             (prin1-to-string 
                                                                                    xcl::label))
                                                          il:bkmenuinfo il:¬ xcl::frame-number
                                                          il:frame-name il:¬ xcl::label)))
...
(defun dbg::attach-backtrace-menu (&optional tty-window skip)
  ...
  (prog (btw bkmenu
     ...
     (setq bkmenu
	   (il:|create| il:menu
	       il:items il:¬ (dbg::collect-backtrace-items tty-window skip)
     ...
     (il:addmenu bkmenu btw (il:|create| il:position
				il:xcoord il:¬ 0 
				il:ycoord il:¬ (- (il:windowprop btw 'il:height)
						  (il:|fetch| (il:menu 
							       il:imageheight)
						      il:|of| bkmenu))))
  ...
  (xcl:with-collection
	...
	(loop (when (null next-frame) (return))
		...
		(xcl:collect (il:|create| il:bkmenuitem
				 ...
				 il:bkmenuinfo il:¬ frame-number
				 il:frame-name il:¬ label-to-use)))
...
(defun il:select.fns.editor (&optional function)
    ...
    (il:menu (il:|create| il:menu
		 il:items il:¬ (cond ((il:ccodep function)
...
(defmethod il:inspect/as/function (name stack-pointer debugger-window)
  ;; Calls an editor on function NAME.  STKP an WINDOW are the stack pointer
...
(defmethod il:inspect/as/function ((name object) stkp window)
