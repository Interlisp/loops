...
(defun expand-defconstructor (class-name name lambda-list supplied-initargs)
  (let ((class (find-class class-name nil))
    ...
    `(progn
	 ...
	 (declare
	   (ignore ,.(specialized-lambda-list-parameters lambda-list)))
	 (error "Constructor ~S not loaded." ',name))

       ,(make-top-level-form name '(eval load)
	  `(load-constructor
	     ',class-name
...
(defmethod get-constructor ((class standard-class) name &optional (error-p t))
...
(defmethod add-constructor ((class standard-class)
			    (constructor constructor))
  (pushnew constructor (class-constructors class)))

(defmethod remove-constructor ((class standard-class)
			       (constructor constructor))
  (setf (class-constructors class)
	(delete constructor (class-constructors class))))
...
(defmethod load-constructor-internal
	   ((class standard-class) name initargs generators)
...
;;;
;;; The actual definitions of maybe-update-constructors and update-constructor
;;; appear in the file std-class, since they are called before this file is
;;; loaded.
;;;
;;; It appears here as well for documentation purposes.  Be sure to keep the
;;; definition here and there in sync.
;;; 
...
;;;   DEFAULTS     the result of calling class-all-default-initargs on class
...
(defmacro define-constructor-code-type (type arglist &body body)
  (let ((fn-name (intern (let ((*package* *the-pcl-package*)
			       (*print-case* :upcase)
			       (*print-gensym* 't))
			   (format nil "CONSTRUCTOR-CODE-GENERATOR ~S" type))
			 *the-pcl-package*)))
    `(progn
...
(defmethod compute-constructor-code ((class standard-class)
				     (constructor constructor))
  (let* ((proto (class-prototype class))
	 (wrapper (class-wrapper class))
	 (defaults (class-all-default-initargs class))
	 (make
    ...
    (let* ((constants (compute-constant-vector class))
	   ...
	   (method-functions
	     (mapcar #'(lambda (method)
		     ...
		     (append (remove *standard-shared-initialize-method*
				     shared)
			     (remove *standard-initialize-instance-method*
				     init)))))
