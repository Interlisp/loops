...
(defun set-funcallable-instance-function (fin new-value)
  (cond ((not (funcallable-instance-p fin))
	 ...
	 (setf (si:lexical-closure-environment fin)
	       (si:lexical-closure-environment new-value))
...
(defun pcl-print-lexical-closure (exp stream slashify-p &optional (depth 0))
  (if (or (eq *print-lexical-closure* exp)
	...
	(funcall *old-print-lexical-closure* exp stream slashify-p depth))
...
(defun allocate-funcallable-instance-1 ()
  (let ((fin (compiler::.primcall 'sys::new-function))
    ...
    (setf (excl::fn¬symdef fin) 'anonymous-fin)
    (setf (excl::fn¬constant fin) init)
    (setf (excl::fn¬code fin)		; this must be before fn¬start
	  (excl::fn¬code mattress-fun))
    (setf (excl::fn¬start fin) (excl::fn¬start mattress-fun))
    (setf (excl::fn¬flags fin) (logior (excl::fn¬flags init)
				       funcallable-instance-flag-bit))
    (setf (excl::fn¬closure fin)
...
(defun funcallable-instance-data-1 (instance data)
  (let ((constant (excl::fn¬closure instance)))
	...
	((> (+ (excl::function-constant-count new-value)
	 ...
	 (set-funcallable-instance-function fin (make-trampoline new-value)))
        (t
         ...
         (setf (excl::fn¬code fin)	; this must be before fn¬start
	       (excl::fn¬code new-value))
         (setf (excl::fn¬start fin) (excl::fn¬start new-value))
         
         (setf (excl::fn¬closure fin) (excl::fn¬closure new-value))
	 ; only replace the symdef slot if the new value is an 
	 ; interned symbol or some other object (like a function spec)
	 (let ((newsym (excl::fn¬symdef new-value)))
	   (excl:if* (and newsym (or (not (symbolp newsym))
				(symbol-package newsym)))
	      then (setf (excl::fn¬symdef fin) newsym)))
         (setf (excl::fn¬formals fin) (excl::fn¬formals new-value))
         (setf (excl::fn¬cframe-size fin) (excl::fn¬cframe-size new-value))
	 (setf (excl::fn¬locals fin) (excl::fn¬locals new-value))
         (setf (excl::fn¬flags fin) (logior (excl::fn¬flags new-value)
                                            funcallable-instance-flag-bit))
...
(defun set-funcallable-instance-function (fin new-value)
  ...
  (cond ((not (functionp new-value))
        ...
        (t
         ;; tack the instance variables at the end of the constant vector
         (setf (excl::fn¬start fin) (excl::fn¬start new-value))
         (setf (excl::fn¬constant fin) (add-instance-vars
                                        (excl::fn¬constant new-value)
                                        (excl::fn¬constant fin)))
         (setf (excl::fn¬closure fin) (excl::fn¬closure new-value))
	 ;; In versions prior to 2.0. comment the next line and any other
	 ;; references to fn¬symdef or fn¬locals.
	 (setf (excl::fn¬symdef fin) (excl::fn¬symdef new-value))
         (setf (excl::fn¬code fin) (excl::fn¬code new-value))
         (setf (excl::fn¬formals fin) (excl::fn¬formals new-value))
         (setf (excl::fn¬cframe-size fin) (excl::fn¬cframe-size new-value))
	 (setf (excl::fn¬locals fin) (excl::fn¬locals new-value))
         (setf (excl::fn¬flags fin) (logior (excl::fn¬flags new-value)
...
(defun funcallable-instance-data-1 (instance data)
  (let ((constant (excl::fn¬constant instance)))
