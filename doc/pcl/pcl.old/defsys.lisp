...
(defvar *pcl-system-date* "5/22/89  Victoria Day PCL")
...
(defvar *pcl-directory*
  #+Genera
   (let ((my-dir (scl:send sys:fdefine-file-pathname :new-pathname
			   :name nil :type nil :version nil)))
     (flet ((subdir (name)
	      (scl:send my-dir :new-pathname :raw-directory
			(append (scl:send my-dir :raw-directory)
				(list name)))))
       (cons my-dir
	     #+Genera-release-7-2       (subdir "rel-7-2")
	     #+Genera-release-7-3       (subdir "rel-7-3") 
	     #+Genera-release-7-4       (subdir "rel-7-4")
	     #+Genera-release-7-5       (subdir "rel-7-5")
	     )))
  #+Lucid                    (pathname "/usr/pcl/")
  #+ExCL                     (pathname "/usr/pcl/")
  #+KCL                      (pathname "/usr/pcl/")
  #+IBCL                     (pathname "/usr/pcl/")
  #+(and DEC common vax VMS) (pathname "")
  #+:CMU                     (pathname "pcl:")
  #+HP                       (pathname "")
  #+Xerox                 (cons (pathname "{dsk}/usr/local/pcl/")
     #+Xerox-Lyric              (pathname "{dsk}/usr/local/pcl/")
     #+Xerox-Medley             (pathname "{dsk}/usr/local/pcl/"))
  #+:gclisp                  (pathname "/pcl/")
  #+pyramid                  (pathname "")
  #+:coral                   (pathname "rodime:coral:pcl:")
  )
...
;  (mki          t               t                (boot defs low fin))
   ...
   (defclass     t               t                (boot defs low fin slots))
   (std-class    t               t                (boot defs low fin slots))
   (points       t               t                (boot defs low))
   (braid1       t               t                (boot defs low fin))
   (fsc          t               t                (slots boot defs low fin))
   (methods      t               t                (boot defs low fin slots))
   (combin       t               t                (boot defs low fin))
   (dcode        t               t                (defs low vector fin slots))
   (precom1      t               t                (defs low fin vector dcode))
   (precom2      t               t                (defs low fin vector dcode))
   (precom3      t               t                (defs low fin vector dcode))
   (precom4      t               t                (defs low fin vector dcode))
   (fixup        t               t                (boot defs low fin))
   (construct    t               t                (boot defs low))
   (env          t               t                (boot defs low fin))
...
(defun edit-pcl ()
  (dolist (f (cadr (get-system 'pcl)))
    (let ((*system-directory* *pcl-directory*))
      (zweifind-file (make-source-pathname (car f))))))
